...
// opacity and blend mode." Mirrors frontend/src/utils/compositing.js; kept
// as a separate copy rather than a shared package since the backend and
// frontend don't otherwise share a module boundary in this project.
...
// (matching how compositors treat blending against a transparent backdrop).
...
// cels: [{layer_id, frame_id, pixel_data}] (pixel_data may be a JSON string or already-parsed 2D array)
export function flattenDocument({ layers, cels, frameId, width, height }) {
  ...
  for (const layer of order) {
    ...
    const cel = cels.find((c) => c.layer_id === layer.id && c.frame_id === frameId);
    if (!cel) continue;
    const grid = typeof cel.pixel_data === 'string' ? JSON.parse(cel.pixel_data) : cel.pixel_data;
